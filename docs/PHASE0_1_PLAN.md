...
**Status (WT-5):** Accounts + watchlist shipped 2026-08-22 (migration `20260822100000_wt5_watchlist`). Passwordless magic-link auth (`lib/auth.ts`, HMAC via Node crypto; `AUTH_SECRET`/`AUTH_MAGIC_SECRET` envs), routes under `/api/auth/*`, session cookie `wt_session` (30d, HttpOnly). The result card's "Watch this for me" CTA (`components/watch-button.tsx`) creates a `WatchItem` (unique per user+obligation, `userNote` added); anonymous users get an email capture → magic link → `/watch?obligation=…` auto-link confirmation → `/watchlist` dashboard with status chips (open/upcoming/due/resolved/dismissed), deadline + exposure per item, resolve/dismiss actions (append-only `Event` rows). Watchlist page server-renders from canonical rows; human deadline labels come from the legacy submission result until WT-3. Dev mode returns the magic link in the API response (no email provider yet — real sending lands in WT-6).

**Status (WT-6):** Transactional email + deadline notifications shipped 2026-08-22. Resend provider (`RESEND_API_KEY`/`EMAIL_FROM`, plain-text-first + single HTML card per DESIGN_LANGUAGE §8) sends real magic-link emails in production (dev keeps token-in-response; e2e uses an in-memory stub behind `NOTIFY_STUB_SENDER=1`). Deadline scheduler: startup + hourly in-process sweep (`instrumentation.ts` → `lib/sweep-scheduler.ts` → `lib/notify-sweep.ts`; manual trigger `GET /api/notify/sweep`) selects watch items due within 7/3/1 days, alert-gated to `verification=certain` + `confidence ≥ 0.9` (§5.4), one email per item per deadline with `notifiedAt`/`Event(type=notified)` dedupe inside a transaction. Emails carry a one-click signed unwatch link (`GET /api/unwatch/[token]` → dismiss + Event). Deadlines are parsed from legacy human labels ("October 14") until WT-3 ships ISO dates.

**Status (WT-14):** Deadline reminder lifecycle shipped 2026-08-25 (migration `20260825194500_wt14_reminder_lifecycle`). Sweep reworked around the watchlist state machine: window is now T-7/T-1 (`NOTIFY_WINDOW_DAYS`, comma-separated, strictest cadence wins); per-cadence dedupe via `WatchItem.lastNotifiedCadence` (restart-safe — a missed T-7 still allows T-1, unlike the old notifiedAt-only guard). Effective deadline order: `WatchItem.deadline` → nearest future `NoticeDeadline` (renamed from the dead WT-4 `Deadline` table; multi-deadline contracts) → `Obligation.dueDate` → legacy label → (recurring-only) `Obligation.renewalDate` for "next renewal in X days" when no hard deadline exists (`renewalReminderEmail` template). Lifecycle: watch creation derives initial status from the deadline; T-7 send advances open → upcoming; T-1 (or deadline passed unreported, `markOverdue`) advances → due; resolve/dismiss stop reminders. Recipient email is read from the watch item's user (anonymous-analyzer obligations have no `userId`, so `obligation.user` was always null). Verified: 246 unit + 19 e2e green; e2e proves a 5-days-out item gets exactly one T-7 reminder, advances to `upcoming`, and a second sweep does not re-send.

**Status (WT-3):** Ingestion + extraction pipeline shipped 2026-08-23 (migration-free; `lib/extraction.ts` + `lib/dates.ts` + `lib/llm-extract.ts`). PDFs/images/.eml uploads are now extracted deterministically and analyzed instead of being queued: PDF text layer via `pdfjs-dist` (legacy build, `serverExternalPackages`), image OCR via `tesseract.js`, `.eml` via an RFC822 parser (quoted-printable/base64 + HTML fallback); unreadable files keep the honest manual-review fallback with the same response contract. Canonical dates are now real ISO `Date` rows (`Obligation.dueDate/renewalDate/noticeDeadlineDate/expiryDate`) via the shared `lib/dates.ts` primitive (human "October 14" labels anchored to next occurrence). Optional env-gated LLM structured extraction (`LLM_API_KEY` + OpenAI-compatible `LLM_BASE_URL`/`LLM_MODEL`): temperature 0, zod-schema-validated with one retry, per-field confidence + provenance quotes, ISO dates, mapped into canonical rows via `toCanonicalStructured`; deterministic risk logic still gates the user-facing result. No key = fully deterministic path (unchanged). `Document.extractionMethod` records `pdf-parse`/`ocr`/`eml`/`raw`/`llm`.

**Status (WT-8):** Security & privacy hardening shipped + pushed 2026-08-22 (commit `9d23fad` → `main`; `FIELD_ENCRYPTION_KEY` set on VPS). Migration `20260822120000_wt8_security_hardening`. Field-level encryption at rest: AES-256-GCM envelope (`v1.iv.tag.ct`, `FIELD_ENCRYPTION_KEY`, dev fallback) applied to `User.email`, `Waitlist.email`, `Submission.content/result/analysis/rawBytes/dataUrl`, `Document.extractedText`; deterministic HMAC `User.emailHash` for email lookups (`lib/crypto.ts`). Consent flow: analyzer requires the checkbox (403 without it), stored as `Submission.consent/consentAt`; `/privacy` page. Rate limiting: per-email magic-link cooldown (1/min) + hourly cap (5), keyed by email hash (fixed the old global-cooldown bug), plus in-memory per-IP quota on analyses (`lib/rate-limit.ts`). Retention: daily in-process sweep (`RETENTION_ENABLED=1`, `lib/retention.ts` + scheduler) deletes unconsented/stale-queued submissions, anonymizes old unreferenced submissions (content/result/rawBytes nulled), keeps watched items; manual trigger `GET /api/admin/retention` (admin bearer). Audit log: append-only `AuditLog` rows for admin auth/review, auth verify success/failure, unwatch, retention (`lib/audit.ts`). Hardening: `Secure` cookies behind https (APP_ORIGIN), security headers (`next.config.ts`), lazy Prisma client in `lib/db.ts` so test DBs don't cross-contaminate. ⚠️ Deploy prerequisite (done): `FIELD_ENCRYPTION_KEY` set on the VPS before this commit landed; legacy plaintext rows remain readable (decrypt returns raw when not an envelope).
...
**Status (2026-08-25):** Phase 1 shipped — all WT-1..WT-9 Done. Phase 2 started; Linear issues WT-10..WT-15. **WT-11 shipped** (commit `48de397`) and **verified live** 2026-08-25: `e2e-inbound.sh` all PASS (both signing secrets, bad-secret 400, GET 405), messages recorded as `InboundMessage` rows (quarantined `unknown_address` — correct until an address exists). **WT-12 shipped** (commit `6473984`): per-account forwarding addresses — `lib/forward-address.ts` (provision/rotate/disable, 128-bit random local part, collision-safe), `POST|GET /api/forwarding`, `/forwarding` settings UI (create/copy/rotate/disable, active badge, received count), linked from the watchlist header. Auto-ingestion was already wired in WT-11's `processInboundWebhook` — with an active `InboundAddress` a forwarded message resolves to its owner and ingests through the WT-3 pipeline (source `forward`, provenance, dedupe, quarantine gates). **WT-13 shipped**: money-protected ledger — `LedgerEntry` rows with strict categories (`prevented`/`recovered`/`avoided`), migration `20260824221313_wt13_ledger`, `lib/ledger.ts` (category math + idempotent recording, `(userId, obligationId, category)` unique), `GET|POST /api/ledger` (same auth as watchlist), `/ledger` dashboard (total + per-category breakdown with definitions + drill-down), watchlist header "Money protected: $X" link, and auto-recording a `prevented` entry when a watch item is resolved. Next: WT-14.

**Status (2026-08-26):** **WT-14 shipped** (commit `9d6c64c`): deadline reminder lifecycle — T-7/T-1 cadence (`NOTIFY_WINDOW_DAYS`), per-cadence dedupe via `WatchItem.lastNotifiedCadence`, `open → upcoming → due` state machine, `NoticeDeadline` model (multi-deadline), renewal-approaching emails for recurring obligations, `markOverdue`, reopen recomputes status. **WT-15 shipped** (this commit): first acquisition experiments — two free vertical SEO tools (`/tools/contract-renewal-analyzer`, `/tools/cancellation-deadline-checker`, `lib/tools.ts`, thin wrappers over the analyzer with preseeded examples), the "Check another document free" repeat hook after every result (landing + tools), and full funnel instrumentation: `ExperimentEvent` rows (`tool_view → analysis_start → result → account_created`, migration `20260826180410_wt15_experiment_events`), `Submission.tool`, anonymous `wt_session_id` cookie joining the funnel, `wt_experiment` source cookie for content CTAs, and an admin Experiments table. Verified: 263 unit + 23 e2e green.

#### Phase 2 work breakdown (Linear WT-10..WT-15)
- **WT-11 (Urgent, do first)**: inbound email infra — forwarding subdomain + MX, SPF, DKIM, DMARC, SMTP→webhook (Resend Inbound), anti-abuse (rate limits, size caps, quarantine), reputation monitoring. Must land before any public inbound mail. **→ shipped (commit `48de397`; verified live 2026-08-25 via `e2e-inbound.sh` — all PASS, rows land).**
- **WT-12**: per-account unique forwarding addresses (`u-<token>@in.watchtower.salmaan.dev`), encrypted at rest (WT-8 crypto), settings UI with rotate/disable, auto-ingestion into the WT-3 pipeline → watch items with provenance. **→ shipped (commit `6473984`).**
- **WT-13**: exposure/money-protected ledger — strict categories (prevented / recovered / avoided), every dollar traceable to an obligation event; dashboard header + drill-down. **→ shipped (migration `20260824221313_wt13_ledger`, `lib/ledger.ts`, `/api/ledger`, `/ledger`).**
- **WT-14**: deadline reminder sweep (T-7 / T-1) wired to the watchlist lifecycle state machine, idempotent, reuses WT-6 Resend transport. **→ shipped (commit `9d6c64c`).**
- **WT-15**: acquisition experiments — short-form content, 2 SEO vertical tools (thin wrappers over the pipeline), "check another document free" repeat hook, landing A/B continues. **→ shipped (this commit): both SEO tools live at `/tools/*`, repeat CTA on every result, `ExperimentEvent` funnel instrumentation + admin table. Content/Reddit experiments are operations (posting), not code — tracking infra is in place.**

**Phase 2 exit criteria:** a user can forward a bill → appears as a watch item; ≥30% of accounts with a watch item return within 30 days; money-protected ledger 100% traceable; at least 2 acquisition experiments complete with data.

**Ordering principle:** WT-11 (infra + trust) before WT-12 (user-facing ingestion); WT-14 depends on the watchlist lifecycle; WT-15 runs in parallel.
