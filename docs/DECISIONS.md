...
## 2026-08-26 — WT-15: First acquisition experiments shipped (SEO tools + repeat hook + funnel instrumentation)

**Decision:** Ship the first two acquisition experiments as product-native surfaces, not marketing pages: two free vertical SEO tools at `/tools/contract-renewal-analyzer` and `/tools/cancellation-deadline-checker` (thin wrappers over the same analyzer pipeline, each with positioning copy + a preseeded example that reliably demonstrates the tool's promise), plus a "Check another document free" repeat CTA after every result on the landing page and tool pages. Every acquisition step is funnel-instrumented with append-only `ExperimentEvent` rows (`tool_view → analysis_start → result → account_created`, migration `20260826180410_wt15_experiment_events`), a `Submission.tool` column, an anonymous `wt_session_id` cookie that joins a visitor's funnel across requests, and a `wt_experiment` source cookie for content CTAs to set (e.g. `?wt_experiment=tiktok`). The admin page gained an Experiments table aggregating per-tool funnel counts. Content/Reddit distribution is operational, not code — the tracking infrastructure is what this issue ships.
